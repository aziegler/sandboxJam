   ...
   /*     var spriteRenderer = (SpriteRenderer)gameObject.GetComponent<SpriteRenderer>();
        ...
        if (null != raycastHit2D.collider)
        {
            ...
            if(null != slot)
            {
                ...
                if (GameManager.Instance.CurrentZoom >= GameManager.Instance.FirstZoomValue && GameManager.Instance.CurrentZoom < GameManager.Instance.FirstZoomValue + GameManager.Instance.SecondZoomValue)
                {
                    if(hitVector.x >0)
                ...
                else if (GameManager.Instance.CurrentZoom >= GameManager.Instance.FirstZoomValue + GameManager.Instance.SecondZoomValue)
                {
                    slot.LeftNeighbor.HitByLaser(hitVector);
                    slot.RightNeighbor.HitByLaser(hitVector);
                    if (hitVector.x > 0)
